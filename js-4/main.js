...
let getLocalDay = function (date) {
  ...
  return res.getDay() == 0 ? 7 : res.getDay();
};
...
let getDateAgo = function (date, days) {
  const reg = new RegExp(",\\s+(\\d.*)", "gi");

  const ndate = new Date(date);
  ndate.setDate(ndate.getDate() - days);

  const res = ndate.toLocaleString().replace(reg, "");
  return res;
};
...
//   info: finfo,
//   get used() {
//     return car.year == yearNow ? "new" : "used";
//   },
//   set used(i) {
//     i == "new" ? (car.year = yearNow) : car.year;
//   },
// };
...
function testPerformance(iterations, func) {
  var date = new Date();
  if (typeof func === "function") for (let i = iterations; i > 0; i--) func();
  // typeof func=== "function" ? for (let i = iterations;i=0;i--) func () : return 0;
