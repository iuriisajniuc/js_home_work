...
console.log(replaceCSSComments("-------------------------")); // код без комментов
...
function validateFileType(str) {
  const reg = new RegExp("\\.(png+$|jpeg+$|jpg+?)", "gi");
  return reg.test(str);
}
...
function checkEmail(str) {
  const reg = new RegExp("^[a-zA-Z-.]+@[a-z]+(\\.[a-z]{2,3}){1,2}$", "gi");
  return reg.test(str);
}
...
function checkDomainUrl(str) {
  const reg = new RegExp("^https?:\\/\\/.+?\\.\\w+$", "gi");
  return reg.test(str);
}
